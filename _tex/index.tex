...
resolved by constraining the variances of random effects to be
positive.} Lastly, Bayesian methods have shown proficiency in handling
...
scores. \emph{Note:} Extracted from Boonen et al.
...
\caption{Fitted models. {\emph{Note:} Yes indicates the feature or
parameter was included in the model, No indicates the
oposite.}}\label{tbl-fitted}\tabularnewline
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\centering
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
Model
...
\texttt{WAIC}, and \texttt{PSIS}. {The figure also includes the
components \texttt{dWAIC} and \texttt{dPSIS}, which reflect the
differences in out-of-sample deviations of the models from the
best-fitting model. Additionally, the figure depicts the associated
uncertainty of the criteria calculated as \(X \pm 1 \cdot\)\texttt{SE}
for \(X=\{\)\texttt{WAIC}, \texttt{PSIS}\(\}\), as well as the
associated uncertainty of comparison calculated as
\(X \pm 1 \cdot\)\texttt{dSE} for \(X=\{\)\texttt{dWAIC},
\texttt{dPSIS}\(\}\).} Table~\ref{tbl-rq1-waic} and
...
\emph{Note:} Black and blue points describe posterior means, and
...
{\emph{Note:} Table sorted from most to least plausible models for the
data.}}
